...
class TestClient:
    ...
    @pytest.mark.asyncio
    async def test_get_abi(self):
        assert await self.client.get_abi(self.thor_router_address) == self.router_abi
        assert await self.client.get_abi(self.thor_token_address) == self.token_abi
